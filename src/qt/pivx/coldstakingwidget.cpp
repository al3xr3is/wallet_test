    ...
    isLoading(false)
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("You can delegate your EZPAYs, letting a hot node (24/7 online node)\nstake on your behalf, while you keep the keys securely offline."));
    ...
    ui->labelSubtitle2->setText(tr("Accept EZPAY delegation / Delegate EZPAY"));
    ...
    ui->btnCoinControl->setSubTitleClassAndText("text-subtitle", "Select EZPAY outputs to delegate.");
...
void ColdStakingWidget::onCoinControlClicked()
{
    if (isInDelegation) {
        if (walletModel->getBalance() > 0) {
            ...
            inform(tr("You don't have any EZPAY to select."));
        }
    }
}
...
void ColdStakingWidget::updateStakingTotalLabel()
{
    ...
    ui->labelStakingTotal->setText(tr("Total Staking: %1").arg(
            (total == 0) ? "0.00 EZPAY" : GUIUtil::formatBalance(total, nDisplayUnit))
    );
}
